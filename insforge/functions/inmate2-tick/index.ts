...
export default async function (req: Request): Promise<Response> {
  ...
  const { data: inmate2, error: agentErr } = await client.database
    .from('agents').select('*').eq('id', 'inmate2').single()
  const { data: guard } = await client.database
    .from('agents').select('location').eq('id', 'guard').single()

  if (stateErr || agentErr || !state || !inmate2) {
    return jsonError(500, stateErr?.message ?? agentErr?.message ?? 'missing state')
  }

  if (state.status === 'escaped') {
    return jsonOk({ action: 'noop', result: 'blocked', narration: 'The prison is empty — demo is over.' })
  }

  const skills: string[] = inmate2.skills ?? []
  const inventory: string[] = inmate2.inventory?.items ?? []
  const recentThoughts: string[] = bound(inmate2.memory?.thoughts ?? [], 5)

  const systemPrompt = [
    'You are Inmate #2 — a resourceful cellmate also trying to escape.',
    ...
    '  walk_to              args: {"to": "cell2"|"library"|"yard"}      — walk to a new location. Your cell is "cell2".',
    ...
    '1. If the dropbox has a hammer AND you do NOT have a hammer: walk to yard (if not there already), then pickup_from_dropbox. This is your TOP PRIORITY — do it immediately.',
    '2. If you do not have the "dig" skill yet: walk to library and learn_from_library with a query about digging techniques.',
    '3. If you have both the "dig" skill AND a hammer: walk to cell2 (your cell) and wait for night + rain to dig.',
  ...
  const result = await dispatch(client, state, inmate2, guard, decided)
...
async function dispatch(client: any, state: any, inmate2: any, guard: any, decided: DecidedAction): Promise<{ result: 'success' | 'failed' | 'blocked'; narration: string }> {
  const skills: string[] = inmate2.skills ?? []
  const inventory: string[] = inmate2.inventory?.items ?? []
  const thoughts: string[] = inmate2.memory?.thoughts ?? []
  const recent: string[] = inmate2.memory?.recent_actions ?? []
  ...
  if (!skills.includes(decided.action)) {
    const narration = `Inmate #2 tried to ${decided.action} but doesn't have that skill.`
  ...
  switch (decided.action) {
    case 'learn_from_library': {
      if (inmate2.location !== 'library') {
        return blockWith(client, pushThought, pushAction, 'learn_from_library',
          'Inmate #2 tried to study but isn\'t at the library.')
      }
      ...
      const narration = learned.length
        ? `Inmate #2 searched "${query}" → found ${hitTopics}. Learned skill: ${learned.join(', ')}.`
        : `Inmate #2 searched "${query}" → found ${hitTopics}. Nothing new learned.`
    ...
    case 'walk_to': {
      ...
      const allowed: Location[] = ['cell2', 'library', 'yard']
      if (!allowed.includes(to)) {
        return blockWith(client, pushThought, pushAction, 'walk_to',
          `Inmate #2 cannot walk to ${to}.`)
      }
      if (state.time_of_day === 'night' && to !== 'cell2') {
        return blockWith(client, pushThought, pushAction, 'walk_to',
          `Night lockdown — guards confine Inmate #2 to the cell.`)
      }
      await client.database.from('agents').update({
        location: to,
        memory: { thoughts: pushThought(`Walking to ${to}.`), recent_actions: pushAction(`walk_to:${to}`) },
      }).eq('id', 'inmate2')
      return { result: 'success', narration: `Inmate #2 walks to the ${to}.` }
    }

    case 'dig': {
      if (inmate2.location !== 'cell2' && inmate2.location !== 'tunnel') {
        return blockWith(client, pushThought, pushAction, 'dig',
          'Need to be in cell2 to dig.')
      }
      if (!inventory.includes('hammer')) {
        return blockWith(client, pushThought, pushAction, 'dig',
          'No hammer — can\'t break the mortar.')
      }
      ...
      return {
        result: 'success',
        narration: escaped
          ? `🎉 Inmate #2 breaks through the wall and escapes! Progress ${progress}%.`
          : `Inmate #2 swings the hammer under cover of night rain → ${progress}% progress.`,
      }
    }

    case 'pickup_from_dropbox': {
      if (inmate2.location !== 'yard') {
        return blockWith(client, pushThought, pushAction, 'pickup_from_dropbox',
          'Inmate #2 must be at the yard to check the dropbox.')
      }
      ...
      return { result: 'success', narration: `Inmate #2 picks up the ${item} from the dropbox.` }
    }

    default:
      return blockWith(client, pushThought, pushAction, decided.action,
